...
<body>
          ...
          <div class="flex flex-col items-center justify-center w-full pt-8 md:pt-12">
               <div class="relative">
                    <a href="edit-profile.php"
                         class="flex items-center justify-center absolute bottom-0 right-0 z-5 p-3 bg-black/80 hover:text-[#f9bb21] duration-300 w-8 h-8 text-white rounded-full">
                         <i class="fa-solid fa-camera text-[12px]"></i>
                    </a>
                    <img src="<?php echo $imageNull == null ? 'media/images/user.png' : $path; ?>"
                         class="rounded-full border-black border-4 md:border-5 shadow-xl w-28 h-28 md:w-32 md:h-32 object-cover" />
               </div>
               ...
               <div
                    class="my-3 font-poppy flex items-center flex-col md:flex-row flex-wrap justify-evenly md:w-1/10 space-y-2 md:space-y-0">
                    ...
                    <?php if ($_SESSION['user_type'] == 0) { ?>
                         <a href="admin.php"
                              class="bg-black cursor-pointer mx-2 py-2 rounded-md px-4 text-white hover:text-[#F9BB21] duration-300">
               ...
               <div
                    class="flex items-center space-x-2 w-full md:w-4/6 md:space-x-16 border-b-[2px] justify-evenly border-black">
                    <a href="user-profile.php"
                         class="w-1/4 justify-center flex items-center hover:bg-black/10 py-3 px-8 duration-500 space-x-2 md:space-x-3 text-xl md:my-0 pb-3 text-dense ">
                         <i class="fa-sharp fa-solid fa-heart text-2xl"></i>
                         <span class="font-poppy font-semibold md:block hidden">Favorites</span>
                    </a>
                    <a href="user-profile.php?review=2"
                         class=" w-1/4 flex justify-center items-center hover:bg-black/10 py-3 px-8 duration-500 space-x-2 md:space-x-3 text-xl md:my-0 pb-3 text-dense">
                         <i class="fa-solid fa-droplet text-2xl"></i>
                         <span class="font-poppy font-semibold md:block hidden"> Drops</span>
                    </a>
                    <a href="user-profile.php?review=1"
                         class="w-1/4 flex justify-center items-center hover:bg-black/10 py-3 px-8 duration-500 space-x-2 md:space-x-3 text-xl md:my-0 pb-3 text-dense">
               ...
               <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 gap-5 place-content-evenly py-8 w-full md:w-4/6">
                    ...
                    if ($result->num_rows > 0) {
                         while ($row = $result->fetch_assoc()) {
                              ...
                              ?>
                              <div class='font-poppy rounded-2xl from-white-op to-black-op  pb-4 px-6 hover:scale-[1.01] hover:shadow-xl duration-500 flex flex-col items-end justify-between'
                    ...
                    <p class=' font-poppy font-medium text-xl text-center md:text-center'>Don't have any favorites yet?
                         <br>Let Eatables help you discover your new go-to dishes!<br><a href="index.php"
                              class="underline">explore more</a>
                    </p>
                    <?php
                    }
          } elseif ($review_id == 1) {
               ...
               <div class="flex flex-row items-center w-3/4 justify-center flex-wrap md:pt-4 ">
                    ...
                    if (mysqli_num_rows($res) > 0) {
                         while ($video = mysqli_fetch_assoc($res)) {
                              ...
                              ?>
                              <a href="drops/view.php">
                                   <div
                                        class="hover:scale-[1.01] duration-500 m-2 relative flex items-center justify-center cursor-pointer">
                                        <video id="video-<?= $video['drop_id'] ?>" class="re playable-video h-[30rem] rounded-md"
                                             data-no-fullscreen="true" src="drops/uploads/<?= $video['video_url'] ?>"></video>
                                        <h1
                                             class="absolute z-50 text-white font-poppy text-bold w-full h-full hover:bg-black/80 hover:rounded-md bg-black/0 flex items-center justify-center duration-500">
                                             <i class="fa-solid fa-heart mr-1"></i><?php echo $video["likes"]; ?></h1>
                                   </div>
                              </a>
                              <?php
                         }
     ...
     <div id="modal" class="modal">
          <div class="modal-content">
               <form action='drops/upload.php' method="post" enctype="multipart/form-data"
                    class="flex flex-col items-center justify-center space-x-2 space-y-3 md:space-y-4 px-4 py-8">
                    <input type="text" name="hotel-name"
                         class="rounded-lg hover:border-brand outline-none opacity-90 border-2 border-black text-xl md:text-2xl px-10 py-2 md:px-16 w-full placeholder:opacity-70 text-center placeholder:font-poppy bg-off-brand placeholder-color font-poppy hover:placeholder:opacity-0 placeholder:duration-[0.5s]"
                         placeholder="Hotel name" required>
                    <input type="file" name="my_video"
                         class="hover:cursor-pointer font-poppy file:py-3 text-center file:border-0 file:px-6 bg-off-brand rounded-xl w-full"
                         required>
                    <button type="submit"
                         class="py-[0.50rem] md:py-[0.70rem] space-x-2 tracking-wider px-9 md:px-12 text-xl font-poppy duration-500 hover:bg-black/30 rounded-xl"
                         name="submit" value="Upload"><i
                              class="fa-solid fa-arrow-up-from-bracket text-xl"></i><span>Upload</span></button>
                    <button type='button' id="close"
                         class="py-[0.50rem] md:py-[0.70rem] tracking-wider px-9 md:px-12 text-xl font-poppy duration-500 close hover:bg-black/30 rounded-xl"
                         onclick="closess();"><i class="fa-solid fa-xmark text-xl"></i></button>
               </form>
          </div>
     </div>
     <script>
